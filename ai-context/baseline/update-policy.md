...
перехода к плоскому локальному слою внутри `ai-context/`.
...
- локальный слой `ai-context/` вне `baseline/` - полностью принадлежит
  конкретному рабочему репозиторию.

Локальный слой включает:
...
- `baseline` можно детерминированно заменять и удалять по состоянию
  source-of-truth;
- локальный слой нельзя перезаписывать при update, если файл уже существует;
- любые project-specific правила, backlog, changelog, repository parameters и
  секреты должны жить только в локальном слое.

## 2. Legacy layout

Старая схема `ai-context/workspace/*` считается legacy-layout.

При sync допускается только одна автоматическая миграция:

- если legacy-путь существует;
- и новый путь еще не существует;
- sync-скрипт переносит содержимое из `ai-context/workspace/*` в
  соответствующий путь внутри `ai-context/*`.
...
Локальный слой всегда имеет project-local приоритет:
...
bootstrap-файлы в локальном слое, если их еще нет.
...
2. при необходимости мигрируй legacy-layout `ai-context/workspace/*`;
3. не трогай существующие файлы локального слоя;
